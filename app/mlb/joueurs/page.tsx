    ...
    <main className="min-h-screen bg-[#0a0d14] text-white">
      ...
      <div className="px-6 py-8 max-w-5xl mx-auto">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          ...
          <section>
            <div className="flex items-center justify-between mb-3">
              <h2 className="text-lg font-bold text-violet-400">🥎 Top lanceurs — ERA</h2>
              <div className="text-xs bg-violet-500/20 text-violet-400 px-2 py-1 rounded-lg">Qualifiés</div>
            </div>
            <div className="bg-[#14171f] border border-[#262b36] rounded-2xl overflow-hidden">
              <div className="grid grid-cols-12 gap-1 px-4 py-2 text-xs text-gray-500 border-b border-[#262b36]">
              ...
              {pitchers.length === 0 ? (
                ...
                pitchers.map((p, i) => {
                  ...
                  const eraColor = era < 2.5 ? 'text-violet-400' : era < 3.5 ? 'text-yellow-400' : era < 4.5 ? 'text-orange-400' : 'text-red-400'
                  return (
                    <div key={p.player.id}
                      className="grid grid-cols-12 gap-1 px-4 py-2.5 text-sm border-b border-[#262b36]/50 last:border-0 items-center hover:bg-gray-800/50 transition-colors">
                      ...
                      <span className="col-span-1 text-center text-violet-400">{p.stat?.wins ?? '—'}</span>
            ...
            <div className="mt-2 p-3 bg-[#14171f] border border-[#262b36] rounded-xl text-xs text-gray-500">
          ...
          <section>
            <div className="flex items-center justify-between mb-3">
              <h2 className="text-lg font-bold text-violet-400">🏏 Top frappeurs — AVG</h2>
              <div className="text-xs bg-violet-500/20 text-violet-400 px-2 py-1 rounded-lg">Qualifiés</div>
            </div>
            <div className="bg-[#14171f] border border-[#262b36] rounded-2xl overflow-hidden">
              <div className="grid grid-cols-12 gap-1 px-4 py-2 text-xs text-gray-500 border-b border-[#262b36]">
              ...
              {hitters.length === 0 ? (
                ...
                hitters.map((h, i) => {
                  ...
                  const avgColor = avg >= 0.300 ? 'text-violet-400' : avg >= 0.270 ? 'text-yellow-400' : 'text-white'
                  return (
                    <div key={h.player.id}
                      className="grid grid-cols-12 gap-1 px-4 py-2.5 text-sm border-b border-[#262b36]/50 last:border-0 items-center hover:bg-gray-800/50 transition-colors">
                      <span className="col-span-1 text-gray-600 text-xs">{i + 1}</span>
        ...
        <div className="mt-8 bg-violet-500/10 border border-violet-500/30 rounded-2xl p-5 flex items-center justify-between gap-4">
          <div>
            <p className="font-bold text-violet-400 mb-1">📖 Tu débutes sur le MLB ?</p>
          ...
          <Link href="/mlb/guide"
            className="shrink-0 bg-violet-500 hover:bg-violet-400 text-black font-bold px-4 py-2.5 rounded-xl transition-colors text-sm">
            Lire le guide →
          </Link>
        </div>
      </div>
