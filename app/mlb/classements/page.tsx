        ...
        {standings.length === 0 ? (
          ...
          <div className="space-y-10">
            {[
              ...
              <section key={label}>
                <h2 className="text-xl font-bold text-violet-400 mb-4">{label}</h2>
                  ...
                  {divs.map(div => (
                    <div key={div.division.id}>
                      ...
                      <div className="bg-[#14171f] border border-[#262b36] rounded-2xl overflow-hidden">
                        {/* Header tableau */}
                        <div className="grid grid-cols-12 gap-2 px-4 py-2 text-xs text-gray-500 border-b border-[#262b36]">
                        ...
                        {div.teamRecords.map((tr, idx) => {
                          ...
                          return (
                            <div key={tr.team.id}
                              className={`grid grid-cols-12 gap-2 px-4 py-3 text-sm items-center border-b border-[#262b36]/50 last:border-0 ${isFirst ? 'bg-violet-500/5' : ''}`}>
                              <div className="col-span-4 flex items-center gap-2">
                                <span className="text-lg">{info?.emoji ?? '⚾'}</span>
                                <div>
                                  <p className={`font-semibold ${isFirst ? 'text-violet-400' : 'text-white'}`}>
                                    {info?.shortName ?? tr.team.abbreviation}
                                  </p>
                                  <p className="text-xs text-gray-600 hidden sm:block">{tr.team.teamName}</p>
                                </div>
                              </div>
                              <span className="col-span-1 text-center font-bold text-violet-400">{tr.wins}</span>
