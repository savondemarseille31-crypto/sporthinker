...
function LoginForm() {
  ...
  const from = searchParams.get('from') ?? '/selections'
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const res = await fetch('/api/auth/picks-login', {
      method:  'POST',
...
export default function SelectionsLogin() {
  return (
    <main className="min-h-screen bg-gray-950 flex items-center justify-center px-4">
      <div className="w-full max-w-sm">
        <div className="text-center mb-8">
          <div className="text-5xl mb-4">🔒</div>
          <h1 className="text-2xl font-bold text-white mb-2">Sélections</h1>
          <p className="text-gray-500 text-sm">Section privée — accès restreint</p>
        </div>
