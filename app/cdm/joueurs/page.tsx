          ...
          <div className="flex flex-wrap gap-2">
            {POSTES.map(p => (
              <button key={p} onClick={() => setFiltrePoste(p)}
                className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-colors ${filtrePoste === p ? 'bg-emerald-500 text-black' : 'bg-gray-800 text-gray-400 hover:border-emerald-500 border border-gray-700'}`}>
                {p}
              </button>
            ))}
          </div>
          <div className="flex items-center gap-4">
            ...
            {[['note', '⭐ Note'], ['buts', '⚽ Buts'], ['passes', '🎯 Passes'], ['xG', '📊 xG']].map(([val, label]) => (
              <button key={val} onClick={() => setTri(val)}
                className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-colors ${tri === val ? 'bg-emerald-500 text-black' : 'bg-gray-800 text-gray-400 border border-gray-700'}`}>
                {label}
              </button>
            ))}
          </div>
            ...
            <Link key={player.id} href={`/cdm/joueurs/${player.id}`}
              ...
              <div className="flex items-start justify-between mb-3">
                ...
                <div className="text-right">
                  <p className="text-2xl font-bold text-emerald-400">{player.note}</p>
