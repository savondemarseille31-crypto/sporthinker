...
const STATUT_COLORS: Record<StatutPari, string> = {
  ...
  gagné: 'bg-violet-500/20 text-violet-400',
      ...
      <div className="px-6 py-8 max-w-4xl mx-auto">
        <div className="flex items-center justify-between mb-6">
          ...
          <Link href="/paris/calculateur"
            className="bg-violet-500 hover:bg-violet-400 text-black font-bold px-4 py-2 rounded-xl text-sm transition-colors">
            + Nouveau pari
          </Link>
        </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-5 gap-3 mb-6">
          {[
            ...
            <div key={s.label} className="bg-[#14171f] border border-[#262b36] rounded-xl p-3 text-center">
        ...
        {evolution.length > 1 && (
          <div className="bg-[#14171f] border border-[#262b36] rounded-2xl p-5 mb-6">
            <div className="flex items-center justify-between mb-3">
              <h2 className="text-sm font-bold text-violet-400">📈 Évolution P&L</h2>
              ...
              {evolution.map((val, i) => {
                ...
                return (
                  <div key={i} className="flex-1 flex flex-col justify-end" title={`${val >= 0 ? '+' : ''}${val.toFixed(2)}${bankroll.devise}`}>
                    <div
                      className={`rounded-sm ${val >= 0 ? 'bg-violet-500' : 'bg-red-500'}`}
        ...
        <div className="flex gap-2 mb-4 flex-wrap">
          {(['tous', 'en_cours', 'gagné', 'perdu', 'annulé'] as const).map(f => (
            <button key={f} onClick={() => setFiltre(f)}
              className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-colors ${filtre === f ? 'bg-violet-500 text-black' : 'bg-gray-800 text-gray-400 border border-gray-700'}`}>
        ...
        {filtered.length === 0 ? (
          <div className="text-center py-12 text-gray-600">
            ...
            {paris.length === 0 && (
              <Link href="/paris/calculateur"
                className="mt-3 inline-block bg-violet-500 hover:bg-violet-400 text-black font-bold px-5 py-2.5 rounded-xl transition-colors text-sm">
          ...
          <div className="space-y-3">
            {filtered.map(pari => (
              <div key={pari.id} className="bg-[#14171f] border border-[#262b36] rounded-2xl p-5 hover:border-gray-700 transition-colors">
                ...
                <div className="grid grid-cols-3 gap-3 mb-3">
                  ...
                  <div className="bg-gray-800 rounded-xl p-2.5 text-center">
                    ...
                    <p className={`font-bold ${
                      pari.statut === 'gagné' ? 'text-violet-400' :
                  ...
                  editId === pari.id ? (
                    <div className="flex gap-2 flex-wrap">
                      <button onClick={() => handleStatut(pari.id, 'gagné')}
                        className="flex-1 bg-violet-500/20 border border-violet-500/40 text-violet-400 font-bold py-2 rounded-xl text-sm hover:bg-violet-500/30 transition-colors">
                        ✅ Gagné
                      </button>
