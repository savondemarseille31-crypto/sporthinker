...
const FORCE: Record<Force, { dot: string; badge: string; label: string }> = {
  fort:           { dot: 'bg-emerald-400', badge: 'bg-emerald-500/20 text-emerald-400 border border-emerald-500/30', label: '⚡ Fort' },
...
export function PaywallNotice({ count }: { count?: number }) {
  return (
    <div className="mb-8 bg-emerald-500/10 border border-emerald-500/30 rounded-2xl p-5 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
      ...
      <Link
        href="/abonnement"
        className="shrink-0 bg-emerald-500 hover:bg-emerald-400 text-black font-bold px-5 py-3 rounded-xl text-sm transition-colors text-center"
      >
        Passe Premium →
      </Link>
    </div>
  ...
  return (
    <main className="min-h-screen bg-gray-950 text-white">
      ...
      <div className="px-6 py-8 max-w-6xl mx-auto">
        <div className="bg-gray-900 border border-gray-800 rounded-2xl p-10 text-center max-w-xl mx-auto my-10">
