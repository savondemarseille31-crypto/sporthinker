      ...
      {form.split('').slice(-3).map((r, i) => (
        <span
          ...
          className={`inline-block w-2 h-2 rounded-full ${
            r === 'W' ? 'bg-violet-400' : r === 'D' ? 'bg-yellow-400' : r === 'L' ? 'bg-red-400' : 'bg-gray-600'
          }`}
        />
      ))}
    </div>
...
function StandingsTable({ standings, live }: { standings: TeamStanding[]; live: boolean }) {
  return (
    <div>
      {/* En-tête */}
      <div className="grid grid-cols-[28px_1fr_36px_28px_28px_28px_28px_36px] gap-x-1 px-3 py-2 text-xs text-gray-500 font-medium uppercase tracking-wide border-b border-[#262b36]">
      ...
      {standings.map((t, idx) => {
        ...
        return (
          <div
            key={t.team}
            className={`grid grid-cols-[28px_1fr_36px_28px_28px_28px_28px_36px] gap-x-1 px-3 py-3 items-center text-sm border-b border-[#262b36]/50 last:border-0 ${
              qualified ? 'bg-violet-500/5' : ''
            }`}
          >
            <span className={`text-center font-bold text-xs ${qualified ? 'text-violet-400' : 'text-gray-500'}`}>
              {t.rank}
            </span>
            ...
            <span className={`text-center font-bold ${qualified ? 'text-violet-400' : 'text-white'}`}>
              {t.pts}
            </span>
            <span className="text-center text-gray-400">{t.played}</span>
            <span className="text-center text-violet-400/80">{t.win}</span>
            <span className="text-center text-yellow-400/70">{t.draw}</span>
            <span className="text-center text-red-400/70">{t.lose}</span>
            <span className={`text-center font-medium text-xs ${t.goalDiff > 0 ? 'text-violet-400' : t.goalDiff < 0 ? 'text-red-400' : 'text-gray-500'}`}>
      ...
      <div className="flex items-center gap-4 px-3 pt-3 pb-1">
        <div className="flex items-center gap-1.5">
          <span className="inline-block w-2 h-2 rounded-full bg-violet-500/40" />
...
export default function GroupesClient({ standings }: { standings: GroupStandingData[] }) {
  ...
  return (
    <>
      ...
      {/* Classement */}
      <div className="bg-[#14171f] border border-[#262b36] rounded-2xl overflow-hidden mb-6">
        <div className="flex items-center justify-between px-5 py-4 border-b border-[#262b36]">
      ...
      {/* Matchs du groupe */}
      <div className="bg-[#14171f] border border-[#262b36] rounded-2xl overflow-hidden mb-10">
        <div className="px-5 py-4 border-b border-[#262b36]">
            ...
            return (
              ...
              >
                <div className="text-xs text-gray-500 w-16 shrink-0">
                  ...
                  {isToday && <span className="block text-violet-400 font-bold">Aujourd'hui</span>}
                </div>
                <span className="text-violet-400 text-xs font-medium w-10 shrink-0">{f.heure}</span>
                <div className="flex-1 flex items-center gap-2 min-w-0">
                  <span>{f.flagD}</span>
                  <span className={`text-sm truncate ${isPast ? 'text-gray-500' : 'text-white group-hover:text-violet-400 transition-colors'}`}>
                    {f.domicile}
                  </span>
        ...
        {groupKeys.map((g) => {
          ...
          return (
            <button
              key={g}
              onClick={() => setActiveGroup(g)}
              className="bg-[#14171f] border border-[#262b36] rounded-xl p-4 text-left hover:border-violet-500 transition-colors"
            >
              ...
              {(gStanding?.standings ?? CDM_GROUPS[g as keyof typeof CDM_GROUPS].teams.map((t, i) => ({ rank: i+1, team: t, pts: 0 }))).map((t) => (
                <p key={t.team} className="text-sm text-gray-400 flex items-center justify-between gap-1">
                  ...
                  {gStanding?.live && (
                    <span className="text-xs font-bold text-violet-400 shrink-0">{t.pts}pts</span>
                  )}
                </p>
              ))}
            </button>
